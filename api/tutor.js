...
function canLeadToMainIdeas(isAbout) {
  const ia = cleanText(isAbout);
  if (
    /(because|so that|so|how|why|caused by|leads to|results in|changed|influenced|prevents|helps|shows|explains|compares|contrasts)/i.test(
      ia
    )
  ) {
    return true;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ error: "Se
