...
// ------------------------------------------------------
// SHADOW INSTRUCTIONAL CONTRACT PATHWAY COMPARISON
// ------------------------------------------------------
//
// Compares the governed contract selected from the
// established Instructional Situation with the contract
// currently selected by the authoritative legacy route.
//
// A difference is not automatically an error.
//
// During migration, the governed pathway may intentionally
// distinguish instructional situations that the legacy
// pathway routes through one general contract.
//
// This comparison does not:
//
// • select either contract;
// • execute either contract;
// • change pending state;
// • change progression;
// • generate communication;
// • transfer authority.
...
function buildShadowInstructionalContractPathwayComparison({
  governedSelection = null,
  legacyContract = null,
} = {}) {
  const governedContractId =
    cleanText(
      governedSelection
        ?.selectedContractId || ""
    );
  ...
  const governedSelectionAvailable =
    Boolean(governedContractId);

  const legacySelectionAvailable =
    Boolean(legacyContractId);

  const contractIdsMatch =
    governedSelectionAvailable &&
    legacySelectionAvailable &&
    governedContractId ===
      legacyContractId;
  ...
  let comparisonReason =
    "One or both contract selections were unavailable.";
  ...
  if (
    ...
    comparisonReason =
      "The governed and legacy pathways selected the same Instructional Contract.";
  } else if (
    governedSelectionAvailable &&
    legacySelectionAvailable
  ) {
    comparisonStatus =
      "differentContractSelection";

    comparisonReason =
      "The governed pathway selected a situation-specific contract while the legacy pathway selected a different contract.";
  } else if (
    governedSelectionAvailable
  ) {
    comparisonStatus =
    ...
    comparisonReason =
      "A governed contract was selected, but no legacy contract was available for comparison.";
  } else if (
    legacySelectionAvailable
  ) {
    comparisonStatus =
    ...
    comparisonReason =
      "A legacy contract was selected, but no governed contract was available for comparison.";
  }

  return {
    artifactType:
      "instructionalContractPathwayComparison",

    version:
      "1.0",

    source:
      "deterministicContractPathwayComparison",
    ...
    governance: {
      comparisonControlsSelection:
        false,

      comparisonControlsExecution:
        false,

      comparisonControlsProgression:
        false,

      comparisonControlsPendingState:
        false,

      comparisonControlsCommunication:
        false,
...
// ------------------------------------------------------
// INTERACTION PATHWAY COMPARISON
// ------------------------------------------------------
//
// Compares the new governed Interaction Instructional
// Finding with the temporary legacy interaction pathway.
//
// This comparison exists only to support migration.
//
// It does not:
//
// • determine instructional truth;
// • classify genuine struggle;
// • select a contract;
// • determine support level;
// • alter progression;
// • alter pending state;
// • generate communication;
// • give legacy reasoning authority over the governed
//   pathway.
//
// A difference is not automatically an error.
//
// The new pathway is intentionally more restrained than
// the legacy pathway. For example:
//
// Governed:
// uncertaintyExpressed
//
// Legacy:
// stuck / struggling / highSupport
//
// These may describe related evidence while operating at
...
async function runMainIdeaSelfTests() {
  ...
  const results = [];
    ...
    // ==================================================
  // MI-GS-001 INSTRUCTIONAL DECISION TESTS
  //
  // Verifies that Main Idea diagnoses deterministically
  // select the Teaching Move, Thinking Move, and
  // communication pattern before AI contextualization.
  //
  // These tests do not call AI and do not change runtime
  // state.
  // ==================================================

  const mainIdeaContract =
    getInstructionalContract(
      "mainIdeas",
      "genuineStruggle"
    );

  const instructionalDecisionTests = [
    {
      name:
        "MI Decision - No evidence reduces cognitive load",
      ...
      expected: {
        ...
        thinkingMoveIncludes:
          "accepted Key Topic and Is About statement",
      },
    },

    {
      name:
        "MI Decision - Insufficient evidence increases specificity",
    ...
    {
      name:
        "MI Decision - Repeated Key Topic differentiates components",
      ...
      expected: {
        ...
        thinkingMoveIncludes:
          "returned to the Key Topic",
      },
    },

    {
      name:
        "MI Decision - Repeated Is About differentiates components",
      ...
      expected: {
        ...
        thinkingMoveIncludes:
          "whole Key Topic is about",
      },
    },

    {
      name:
        "MI Decision - Detail moves to broader organizer",
    ...
    {
      name:
        "MI Decision - Unestablished relationship refocuses",
    ...
    {
      name:
        "MI Decision - Undetermined relationship preserves uncertainty",
      ...
      expected: {
        ...
        thinkingMoveIncludes:
          "Preserve the undetermined relationship",
      },
    },
  ];

  instructionalDecisionTests.forEach(
    (test) => {
      const actual =
        selectMIGS001InstructionalDecision(
          test.finding,
      ...
      const passed =
        actual?.teachingMove ===
          test.expected.teachingMove &&

        actual?.communicationPattern ===
          test.expected
            .communicationPattern &&
...
async function applyIsAboutCapture(s, msg) {
  ...
  refreshShadowInstructionalSituationWithComponentFinding({
    ...
    s.instructionalContractPathwayComparison =
      structuredClone(
        instructionalContractPathwayComparison
      );
    ...
    if (
      ...
      typeof s.instructionalAssessment ===
        "object"
    ) {
      s.instructionalAssessment
        .instructionalContractPathwayComparison =
...
// Attach the temporary migration comparison.
//
// This artifact exists only to measure whether the new
// governed pathway covers the useful responsibilities of
// the legacy pathway before that legacy pathway is removed.
instructionalAssessment
  .interactionPathwayComparison =
    structuredClone(
      interactionPathwayComparison
    );
...
// during migration. This field will be removed when the
// legacy pathway is deleted.
s.interactionPathwayComparison =
  structuredClone(
    interactionPathwayComparison
  );
